  ...
  return (
    <div className="w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        <div>
           <h1 className="text-2xl font-bold text-gray-800 dark:text-slate-200 uppercase">Closed Accounts (बंद खाते)</h1>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-[#0B1120] shadow-md border border-gray-300 dark:border-slate-600">
        ...
        <div className="bg-gray-800 text-white p-6 border-b-4 border-gray-600 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
             <div className="w-14 h-14 bg-white dark:bg-[#0B1120] rounded-full flex items-center justify-center shrink-0">
                <Archive className="w-8 h-8 text-gray-800 dark:text-slate-200" />
             </div>
             <div>
          ...
          <div className="relative w-full sm:w-[320px]">
             <Search className="absolute left-3 top-2.5 w-5 h-5 text-gray-800 dark:text-slate-200" />
             <input
               type="text"
               placeholder="Search by name or Account No..."
               value={searchTerm}
               onChange={(e) => setSearchTerm(e.target.value)}
               className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-[#0B1120] border border-gray-600 rounded-md focus:ring-2 focus:ring-gray-400 focus:outline-none text-[14px] font-bold text-gray-800 dark:text-slate-200 placeholder-gray-500 shadow-inner"
             />
          </div>
        </div>

        {/* Status Strip */}
        <div className="bg-gray-100 dark:bg-slate-700/50 p-3 border-b border-gray-300 dark:border-slate-600 flex justify-between items-center px-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
             <thead className="bg-gray-50/80 dark:bg-slate-700/50 border-b border-gray-100 dark:border-slate-700 uppercase text-[11px] font-extrabold tracking-wider text-gray-500 dark:text-gray-300">
                <tr>
             ...
             <tbody className="bg-white dark:bg-[#0B1120]">
                {filteredCustomers.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="p-10 text-center text-gray-500 dark:text-gray-300 font-bold uppercase border border-gray-200 dark:border-slate-600">
                      No Records Found.
                    </td>
                  </tr>
                ) : (
                   filteredCustomers.map((customer, index) => {
                      ...
                      return (
                      <tr key={customer.id} className="border-b border-gray-50 dark:border-slate-800 hover:bg-gray-50/80 dark:hover:bg-slate-800/50 transition-colors group">
                         <td className="p-4 text-gray-400 font-bold w-12 text-center group-hover:text-gray-600 dark:text-gray-300 transition-colors">
                            {(index + 1).toString().padStart(3, '0')}
                         </td>
                         <td className="p-4 font-bold text-gray-600 dark:text-gray-300 uppercase">
                            {displayName}
                         </td>
                         ...
                         <td className="p-4 text-center">
                            <span className="px-3 py-1 rounded-full text-xs font-bold shadow-sm bg-gray-100 dark:bg-slate-700/50 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-slate-600">
                               Closed (बंद)
                            </span>
                         </td>
                         <td className="p-4 text-right">
                            <div className="flex items-center justify-end gap-2">
                               ...
                               <button
                                 ...
                                 className="flex items-center gap-1.5 px-4 py-2 bg-white dark:bg-[#0B1120] border border-red-200 text-red-600 hover:bg-red-50 hover:border-red-300 text-[12px] font-bold rounded-lg shadow-sm hover:shadow-md transition-all hover:-translate-y-0.5"
        ...
        <div className="p-4 bg-gray-50 dark:bg-slate-700/50 border-t border-gray-200 dark:border-slate-600 text-center">
           <p className="text-[11px] text-gray-500 dark:text-gray-300 font-medium uppercase tracking-wide">
              *** END OF DATABASE ***
           </p>
        </div>
      ...
      {customerToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[#0B1120] rounded-md w-full max-w-md overflow-hidden shadow-2xl border-t-4 border-red-600">
            <div className="bg-red-50 p-6 flex flex-col items-center text-center border-b border-red-100">
              <div className="w-16 h-16 bg-white dark:bg-[#0B1120] text-red-600 rounded-full flex items-center justify-center mb-4 shadow-sm">
                <AlertTriangle className="w-8 h-8" />
              </div>
              <h3 className="text-xl font-extrabold text-red-900 uppercase tracking-wide mb-1">Permanent Deletion</h3>
            </div>
            <div className="p-6 text-center">
              <p className="text-gray-700 dark:text-slate-300 text-[14px] font-medium leading-relaxed">
                Are you sure you want to PERMANENTLY delete the account for <strong className="text-gray-900 dark:text-slate-100 uppercase">{customerToDelete.full_name}</strong>?
              </p>
            ...
            <div className="p-4 bg-gray-100 dark:bg-slate-700/50 border-t border-gray-200 dark:border-slate-600 flex gap-3">
              <button
                onClick={() => { setCustomerToDelete(null); setErrorMsg(''); }}
                disabled={isPending}
                className="flex-1 py-2 px-4 bg-white dark:bg-[#0B1120] border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-300 font-bold rounded shadow-sm hover:bg-gray-50 dark:bg-slate-700/50 transition-colors disabled:opacity-50"
              >
                CANCEL
              </button>
              <button
      ...
      {customerToRestore && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[#0B1120] rounded-md w-full max-w-md overflow-hidden shadow-2xl border-t-4 border-emerald-600">
            <div className="bg-emerald-50 p-6 flex flex-col items-center text-center border-b border-emerald-100">
              <div className="w-16 h-16 bg-white dark:bg-[#0B1120] text-emerald-600 rounded-full flex items-center justify-center mb-4 shadow-sm">
            ...
            <div className="p-6 text-center">
              <p className="text-gray-700 dark:text-slate-300 text-[14px] font-medium leading-relaxed">
                Are you sure you want to restore the account for <strong className="text-gray-900 dark:text-slate-100 uppercase">{customerToRestore.full_name}</strong>?
              </p>
              {errorMsg && (
                <div className="mt-4 p-3 bg-red-50 text-red-700 text-sm font-bold border border-red-200 w-full">
                  {errorMsg}
