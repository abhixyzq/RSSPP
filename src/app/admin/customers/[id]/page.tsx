  ...
  return (
    <div className="max-w-[1200px] mx-auto pb-20 p-4 sm:p-8 font-sans">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
         <Link href="/admin/customers" className="flex items-center gap-2 text-gray-500 hover:text-blue-600 transition-colors font-semibold bg-white px-4 py-2 rounded-lg border border-gray-200 shadow-sm">
      ...
      <div className="bg-white shadow-md border border-gray-300">
         ...
         <div className="p-6 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6 border-b border-gray-200 bg-gray-50/50">
            <div className="space-y-3">
               <div className="grid grid-cols-3">
                  ...
                  <span className="text-sm font-bold text-[#0B2E59] col-span-2 uppercase">
                    {profile.full_name}
               ...
               <div className="grid grid-cols-3">
                  ...
                  <span className="text-sm font-bold text-gray-900 col-span-2 uppercase">
                    {profile.address || 'N/A'}
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-6 bg-transparent">
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex items-center gap-4 group">
               ...
               <div>
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-wider group-hover:text-green-600 transition-colors">Deposit Balance (कुल बचत)</p>
                  <p className="text-2xl font-black text-gray-900 mt-0.5">₹{currentJamaBalance.toLocaleString('en-IN')}</p>
               </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex items-center gap-4 group">
               ...
               <div>
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-wider group-hover:text-red-600 transition-colors">Loan Balance (कुल ऋण)</p>
                  <p className="text-2xl font-black text-gray-900 mt-0.5">₹{currentNikasiBalance.toLocaleString('en-IN')}</p>
               </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex items-center gap-4 group">
               ...
               <div>
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-wider group-hover:text-purple-600 transition-colors">Interest Earned (कुल ब्याज मिला)</p>
                  <p className="text-2xl font-black text-gray-900 mt-0.5">₹{totalJamaInterest.toLocaleString('en-IN')}</p>
               </div>
            </div>
         </div>
            ...
            <table className="w-full text-left border-collapse min-w-[700px]">
               <thead>
                  <tr className="bg-gray-100 text-gray-700 text-[12px] uppercase tracking-wider border-b-2 border-gray-300">
                     <th className="p-3 font-bold border-r border-gray-300 w-32">Date (दिनांक)</th>
                     <th className="p-3 font-bold border-r border-gray-300">Particulars (विवरण)</th>
                     <th className="p-3 font-bold border-r border-gray-300 w-32 text-right text-purple-800 bg-purple-50">Interest Earned (ब्याज मिला)</th>
                     <th className="p-3 font-bold border-r border-gray-300 w-40 text-right">Amount (राशि) (+ / -)</th>
                    ...
                    jamaTransactions.map((tx, index) => {
                      ...
                      return (
                        <tr key={tx.id} className={`${bgClass} hover:bg-green-50/50 text-[13px] text-gray-900 border-b border-gray-200`}>
                           ...
                           <td className="p-3 border-r border-gray-200 text-right font-extrabold text-purple-700 bg-purple-50/30">
                              {isInterest ? `+${interestString}` : '-'}
                           </td>
                           <td className={`p-3 border-r border-gray-200 text-right font-bold ${isInterest ? 'text-gray-400' : isDeposit ? 'text-green-700' : 'text-red-600'}`}>
            ...
            <table className="w-full text-left border-collapse min-w-[700px]">
               <thead>
                  <tr className="bg-gray-100 text-gray-700 text-[12px] uppercase tracking-wider border-b-2 border-gray-300">
                     <th className="p-3 font-bold border-r border-gray-300 w-32">Date (दिनांक)</th>
                     <th className="p-3 font-bold border-r border-gray-300">Particulars (विवरण)</th>
                     <th className="p-3 font-bold border-r border-gray-300 w-32 text-right">Principal (मूलधन) (+/-)</th>
                     <th className="p-3 font-bold border-r border-gray-300 w-32 text-right">Interest Paid (ब्याज दिया)</th>
                    ...
                    nikasiTransactions.map((tx, index) => {
                      ...
                      return (
                        <tr key={tx.id} className={`${bgClass} hover:bg-red-50/50 text-[13px] text-gray-900 border-b border-gray-200`}>
                           ...
                           <td className={`p-3 border-r border-gray-200 text-right font-bold ${isLoanGiven ? 'text-red-700' : isPrincipalRepay ? 'text-green-700' : 'text-gray-400'}`}>
                              {isLoanGiven ? `+ ${amtString}` : isPrincipalRepay ? `- ${amtString}` : '-'}
                           </td>
                           <td className={`p-3 border-r border-gray-200 text-right font-bold ${isInterest ? 'text-purple-700' : 'text-gray-400'}`}>
